...
bool LegKinematics::init()
{
	...
	node_private.param("root_name", root_name, std::string("body"));
...
bool LegKinematics::getLegIKSolver(taticoma_msgs::GetLegIKSolver::Request &request,
								   taticoma_msgs::GetLegIKSolver::Response &response)
{

	taticoma_msgs::LegPositionState leg_dest_pos;
	...
	for (int i = 0; i < request.leg_number.size(); i++)
	{
		...
		if (ik_valid >= 0)
		{
			taticoma_msgs::LegJointsState jnt_buf;
			for (unsigned int j = 0; j < num_joints; j++)
			{

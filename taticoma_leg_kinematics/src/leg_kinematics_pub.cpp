...
int main(int argc, char **argv)
{
	...
	ros::Publisher leg_msg_pub = node.advertise<taticoma_msgs::LegJointsState>("leg_states", 1);
...
//	ik_solver_vel -> setWeightTS(matrix_Mx);
	...
	joint_min(0) = -1.657;	joint_min(1) = -1.657;	joint_min(2) = -1.657;
	joint_max (0) = 1.657;	joint_max (1) = 1.657;	joint_max (2) = 1.657;
	...
	taticoma_msgs::LegJointsState leg_msg;
	...
	while (ros::ok()){
		...
		if (ik_valid >= 0){
			...
			joint_msg.name.push_back("coha_joint_r1");

...
bool GaitKinematics::init()
{
	...
	node.param("trapezoid_low_radius", trap_low_r, 0.045);
	node.param("trapezoid_high_radius", trap_high_r, 0.0345);
	node.param("trapezoid_h", trap_h, 0.025);
	node.param("clearance", trap_z, 0.01);
	...
	joints_pub = node.advertise<taticoma_msgs::LegsJointsState>("/my/joints_to_controller", 1);
	gait_control_sub = node.subscribe<taticoma_msgs::GaitCommand>("/teleop/gait_control", 1, &GaitKinematics::teleopGaitCtrl, this);
...
void GaitKinematics::teleopGaitCtrl(const taticoma_msgs::GaitCommandConstPtr &gait_cmd)
{
